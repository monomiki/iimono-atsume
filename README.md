...
公開サイトは完全な静的HTMLですが、Favorite処理は`favorite-api/`のCloudflare Workers雛形で受けます。
...
`wrangler.toml`の`PUBLIC_DATA_BASE_URL`、`FAVORITE_ALLOWED_ORIGIN`、KV namespace IDは実際の環境に合わせて設定します。Discord送信に失敗したFavoriteは`pending`として保存され、ローカル側は次で再送できます。

```bash
python -m src.cli resend-pending-favorites
